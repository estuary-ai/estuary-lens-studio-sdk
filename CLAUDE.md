...
| session_timeout | Implemented | Server idle-timeout (no conversation activity). Handler flags the server's close as intentional so `autoReconnect` is suppressed — auto-reconnecting after a reap would re-authenticate and re-establish billed backend resources in a loop. Re-emitted as `sessionTimeout` on EstuaryClient (raw contract payload). Resume = explicit `connect()` on user intent. |
