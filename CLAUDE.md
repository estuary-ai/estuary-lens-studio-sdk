...
| client_action | Implemented | Typed in-world action calls (SCRUM-202, contract v1.10), fire-on-arrival per contract (not synced to TTS playback). **Opt-in required:** the auth payload now carries `capabilities: {version: '1', client_action: true}` — this was the SDK's first capability declaration of any kind. The server defaults that field to false and serves the retired XML tag path without it, so omitting it silently reverts Spectacles to name-only actions. Device fields stay omitted (server defaults them true, correct for Spectacles). Forwarding: `EstuaryClient` parses `client_action` → `clientAction` → `EstuaryManager` → `IEstuaryCharacterHandler.handleClientAction?` (optional, backward compatible) → `EstuaryCharacter` re-emits `clientAction` → `EstuaryActionManager` runs the SAME strictMode filter + dispatch as the tag path, so the existing `actionTriggered` / `action:{name}` / `actionsParsed` callbacks fire unchanged — integrators see no API change. `ParsedAction` gains additive optional `params` (argument values stringified to match legacy XML attribute behavior); for typed events `tag` holds a synthesized name-only `<action name="..." />`. This makes parameterized actions work on Spectacles for the FIRST time — the legacy regex only matched name-only tags. The legacy XML tag parser is retained but dormant (fires only on stray tags still in `bot_response.text`). It is now also the live fallback for any *older* lens build that never declares the capability, so removal is gated on the server-side `event=legacy_action_tags` metering rather than a fixed window (SCRUM-203). |
| audio_playback_tracking | Implemented (app-driven) | `notifyAudioPlaybackComplete()` exists on both `EstuaryClient` and `EstuaryManager`, but nothing inside the SDK calls it — playback lives in the app's `DynamicAudioOutput`, so the lens must emit it when its own queue drains. That matters more since contract v1.13 (SCRUM-234): on the WebSocket audio path this event is what ends the character's speaking turn, arming barge-in and anchoring the anti-echo cooldown. A lens that never emits it falls back to the server's estimate (duration of the PCM sent), which is correct-ish but blind to client-side start latency. |
